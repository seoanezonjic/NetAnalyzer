...
class BaseNetTestCase(unittest.TestCase):
	...
	def ranked_genes2array(self, ranked_genes):
		...
		for seed_name, rank_list in ranked_genes.items():
			for ranked_genes in rank_list:
				ranked_genes = []
				test_ranked_genes.append([str(el) for el in ranked_genes] + [seed_name])
		return test_ranked_genes

	def load_ranking(self, file):
		ranked_genes = {}
		f = open(os.path.join(DATA_TEST_PATH, file_name), 'r')
		for line in f:
			fields = line.rstrip().split("\t")
	...
	def setUp(self):
		...
		self.ranker_with_ranking = Ranker.new()
	...
	def test_leave_one_out(self):
		...
		test_ranked_genes = ranked_genes2array(self.ranker.ranking)
	...
	def test_get_top(self):
		test_top_ranked_genes = ranked_genes2array(self.ranker_with_ranking.get_top(2))
		self.assertEqual(self.load_results('top_results'),test_top_ranked_genes)

	def test_filtered_top_compatibility(self):
		filtered_ranked_genes = ranked_genes2array(self.ranker_with_ranking.get_reference_ranks)
		test_top_ranked_genes = ranked_genes2array(self.ranker_with_ranking.get_top(2))
		self.assertEqual(self.load_results('top_results'),test_top_ranked_genes)
